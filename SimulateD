...
public class SimulateD {
	...
	public static double oneSimulationD1() {

		double total = 0;
		double currentPrice = ZEROPRICE*1.4;
		for (int i = 1; i <= 10; i++) {
			double thisyear = presentValue(i, currentPrice);
			total += thisyear;
			currentPrice = nextPrice(currentPrice);
	...
	public static double oneSimulationD2() {

		double total = 0;
		double currentPrice = ZEROPRICE*1.4*1.4;
		for (int i = 2; i <= 10; i++) {
			double thisyear = presentValue(i, currentPrice);
			total += thisyear;
			currentPrice = nextPrice(currentPrice);
	...
	public static void main(String[] args) {
		...
		// giving PV starting with year 1 if price goes up
		simulatedSum = 0;
		for (int i = 0; i < simNumber; i++) {
			simulatedSum += oneSimulationD1();
		}
		presVal = simulatedSum / simNumber;
		System.out.println(presVal);

		// giving PV starting with year 2 if price goes up twice
		simulatedSum = 0;
		for (int i = 0; i < simNumber; i++) {
			simulatedSum += oneSimulationD2();
		}
		presVal = simulatedSum / simNumber;
		System.out.println(presVal);
	}
